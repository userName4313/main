...
def setup():
    global player, ground, cloud_x, cloud_y, cloud, cloud2, cloud2_y, cloud_x2, menuEnabled, menuBack, maze,playerX, moving, ballX, ballY, ballZ, ballVelX, ballVelY, ballVelZ, aiX
...
def draw():
    global GameMode, menuEnabled, player_x, player_y, ground_x, ground_y,dy_ground, player_dx, player_dy, cloud_y, cloud_x, gameEnd, cloud2_y, cloud2_x, playerPower
...
def FallingGame():
    ...
    if cloud_y < -100:
        ...
        cloud_x=random.randint(int(max(player_x-5*(400/(10*dy_ground)), 0)),int(min(player_x+5*(400/(10*dy_ground)),750)))
        #print(cloud_x)
...
def Mayangame():
    global playerX, playerVelX, ballX, ballY, ballZ, ballVelX, ballVelY,aiX, ballVelZ, GameMode, menuEnabled
    #image(image, x, y)
    ...
    if ballY>=360 and abs(playerX+25-ballX)<50:
